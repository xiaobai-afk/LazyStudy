...
let challengeQuestionLoop_ = require("./challengeAnswer.js");
let dailyQuestionLoop_ = require("./dailyAnswer.js");
...
function articleStudy(x) {
    while (!text("工作").exists()); //等待加载出主页
    var listView = className("ListView"); //获取文章ListView控件用于翻页
    if (x == 0) {
        //text("工作").findOnce().parent().parent().parent().click(); //点击主页正下方的"学习"按钮
        id("cn.xuexi.android:id/home_bottom_tab_button_work").click();
        delay(2);
        click(aCatlog);
    }
    delay(2);
    ...
    for (var i = 0, t = 0; i < aCount;) {
        try {
            if ((id("cn.xuexi.android:id/general_card_title_id").findOnce(t).parent().parent().click() || id("cn.xuexi.android:id/general_card_title_id").findOnce(t).parent().parent().parent().click()) == true) {
                delay(3);
                //获取当前正在阅读的文章标题
                let n = 0;
                ...
                if (text("展开").exists()) //如果存在“展开”则认为进入了文章栏中的视频界面需退出
                {
                    ...
                    t++;
                    className("TextView").findOnce(3).parent().parent().child(3).click();
                    delay(1);
                    back();
                    delay(1);
                    continue;
...
function videoStudy_news() {
    delay(1)
    //text("工作").findOnce().parent().parent().parent().click();
    id("cn.xuexi.android:id/home_bottom_tab_button_work").click();
    delay(2)
    click("电视台");
    delay(1)
...
function listenToRadio() {
    ...
    click("听新闻广播");
    delay(2);
    let playIcon = id("cn.xuexi.android:id/lay_state_icon").findOnce().child(0);
    if (playIcon.id() != "cn.xuexi.android:id/v_playing") {
        playIcon.click();
    }
    // id("cn.xuexi.android:id/lay_state_icon").findOnce().child(0).click();
    delay(1);
    //back();
}
...
function challengeQuestionLoop(conNum) {
    ...
    challengeQuestionLoop_(false);
}

function dailyQuestionLoop() {
    dailyQuestionLoop_(false);
}
/**
 * @description: 挑战答题
 * @param: null
